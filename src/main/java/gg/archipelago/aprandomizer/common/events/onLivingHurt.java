...
public class onLivingHurt {
    ...
    static void onLivingHurtEvent(LivingHurtEvent event) {
            ...
            if (entity.getPassengers().size() > 0) {
                if (entity.getPassengers().get(0) instanceof ServerPlayer) {
                    if (event.getSource().getMsgId().equals("fall")) {
                        ...
                        Advancement advancement = event.getEntity().getServer().getAdvancements()
                                .getAdvancement(new ResourceLocation("aprandomizer:archipelago/ride_pig"));
        ...
        if (e instanceof ServerPlayer) {
            ...
            if (event.getAmount() >= 18 && !event.getSource().is(DamageTypes.EXPLOSION)
                    && !event.getSource().getMsgId().equals("fireball")) {
                Advancement a = event.getEntity().getServer().getAdvancements()
                        .getAdvancement(new ResourceLocation("aprandomizer:archipelago/overkill"));
